...
*Galilean transformation* for example is the set of equation that relates the coordinates of two frames of reference that are in constant velocity with respect to each other. If $x$,$y$,$z$,$t$ are the coordinates of an event at a frame of reference and $x'$,$y'$,$z'$,$t'$ are the coordinates of same event at another frame of reference at $v$ velocity along x-axis, with all other spatial coordinate axes parallel with each other, then the transformation transformation equation is:
